 ...
 <body>
  <div class="mws-panel grid_8"> 
   <div class="mws-panel-header"> 
    <span><i class="icon-table"></i> 子轮播管理列表</span> 
   </div> 
    ...
    <div id="DataTables_Table_1_wrapper" class="dataTables_wrapper" role="grid">
     ...
     <table class="mws-datatable-fn mws-table dataTable" id="DataTables_Table_1" aria-describedby="DataTables_Table_1_info"> 
      ...
      <tbody role="alert" aria-live="polite" aria-relevant="all">
       @foreach($spic as $link)
       <tr class="odd"> 
         
        <td class="  ">{{$link->id}}</td> 
        <td class=" ">{{$link->name}}</td> 
        <td class=" "><img src="{{$link->spic}}" style="height:80px"></td> 
       
        <td>
          <form action="/Admin/spic/{{$link->id}}" method="post">
            {{csrf_field()}}
            {{method_field("DELETE")}}
            <button class="btn btn-success" type="submit"><i class="icon-trash"></i></button>
             <a href="/Admin/spic/{{$link->id}}/edit" class="btn btn-success">修改</a>
          </form>
